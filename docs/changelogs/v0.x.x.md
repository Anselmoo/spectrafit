...
- Focus on maintenance fixes for the `SpectraFit` package:
...
- The original program `fastfit` is now running as `SpectraFit` with its own
  installer based on [POETRY](https://python-poetry.org).
