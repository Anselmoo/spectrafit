...
class SpectraFitMCPServer:
            ...
            try:
                ...
                if not data_str:
                    return [types.TextContent(
                        type="text",
                        text="Error: No data provided. Please provide CSV data with 'Energy' and 'Intensity' columns."
                    )]
                ...
                if not model_config:
                    return [types.TextContent(
                        type="text",
                        text="Error: No model configuration provided."
                    )]
                ...
                return [types.TextContent(
                    type="text",
                    text=f"Error performing spectral fit: {str(e)}"
                )]
        
        @self.server.call_tool()
        async def auto_peak_detection(arguments: dict[str, Any]) -> list[types.TextContent]:
            ...
            try:
                ...
                prominence = arguments.get("prominence", None)
                height = arguments.get("height", None)
                distance = arguments.get("distance", None)
                ...
                # Format results
                results = f"Peak Detection Results:\n"
                ...
                return [types.TextContent(
                    type="text",
                    text=f"Error in peak detection: {str(e)}"
                )]
        
        @self.server.call_tool()
        async def generate_fit_model(arguments: dict[str, Any]) -> list[types.TextContent]:
            ...
            try:
                ...
                energy_range = arguments.get("energy_range", None)
                ...
                return [types.TextContent(
                    type="text",
                    text=f"Error generating model template: {str(e)}"
                )]
        
        @self.server.call_tool()
        async def validate_spectral_data(arguments: dict[str, Any]) -> list[types.TextContent]:
            ...
            try:
                ...
                if not data_str:
                    return [types.TextContent(
                        type="text",
                        text="Error: No data provided for validation."
                    )]
                ...
                return [types.TextContent(
                    type="text",
                    text=f"Error validating data: {str(e)}"
                )]
    ...
    def _setup_resources(self) -> None:
        ...
        async def handle_list_resources() -> list[types.Resource]:
            ...
            return [
                types.Resource(
                    ...
                    description="JSON list of available peak and background functions in SpectraFit",
        ...
        async def handle_read_resource(uri: str) -> str:
            ...
            elif uri == "spectrafit://examples/sample-data":
                return self._get_sample_data()
            elif uri == "spectrafit://config/default-parameters":
    ...
    def _format_fit_results(self, fit_results: dict[str, Any]) -> str:
        ...
        if "fit_insights" in fit_results:
            ...
            if "statistics" in insights:
                ...
                results.append(f"  Reduced χ² = {stats.get('reduced_chi_squared', 'N/A')}")
    ...
    def _validate_data_quality(self, df: pd.DataFrame) -> dict[str, dict[str, dict[str, Any]]]:
        ...
        validation["format"]["required_columns"] = {
            "passed": has_required,
            "message": "Has Energy and Intensity columns" if has_required else "Missing required columns"
        }
        ...
        if has_required:
            ...
            validation["format"]["numeric_data"] = {
                ...
                "message": "Data columns are numeric" if energy_numeric and intensity_numeric else "Non-numeric data detected"
            }
            
            # Data quality checks
            energy_monotonic = df["Energy"].is_monotonic_increasing or df["Energy"].is_monotonic_decreasing
            validation["data_quality"]["energy_monotonic"] = {
                "passed": energy_monotonic,
                "message": "Energy values are monotonic" if energy_monotonic else "Energy values are not monotonic"
            }
            
            has_nan = df[["Energy", "Intensity"]].isna().any().any()
            validation["data_quality"]["no_missing_values"] = {
                "passed": not has_nan,
                "message": "No missing values" if not has_nan else "Contains missing values"
            }
            
            # Check data range
            num_points = len(df)
            validation["data_quality"]["sufficient_points"] = {
                "passed": num_points >= 10,
                "message": f"{num_points} data points" + (" (sufficient)" if num_points >= 10 else " (consider more data)")
            }
        
        return validation
    
    def _validate_technique_data(self, df: pd.DataFrame, technique: str) -> dict[str, dict[str, dict[str, Any]]]:
        ...
        if technique in technique_ranges and "Energy" in df.columns:
            ...
            in_range = (energy_min >= spec["min"] - 50) and (energy_max <= spec["max"] + 50)
            validation["technique_specific"]["energy_range"] = {
                "passed": in_range,
                "message": f"Energy range {energy_min:.1f}-{energy_max:.1f} {spec['unit']}" + 
                          (" is appropriate" if in_range else f" (expected {spec['min']}-{spec['max']} {spec['unit']})")
            }
    ...
    def _get_fitting_functions(self) -> str:
        ...
        functions = {
            ...
            "background_functions": [
                "linear", "constant", "exponential", "power",
                "polynom2", "polynom3"
            ],
            "step_functions": [
                "atan_step", "erf_step", "log_step", "heaviside"
            ],
            "convolution_functions": [
                "cgaussian", "clorentzian", "cvoigt"
            ],
            "moessbauer_functions": [
                "moessbauer_singlet", "moessbauer_doublet",
                "moessbauer_sextet", "moessbauer_octet"
            ]
        }
    ...
    def _get_sample_data(self) -> str:
        ...
        intensity = (
            ...
            np.random.normal(0, 2, len(energy)) +        # Noise
            5                                            # Baseline
        )
...
def command_line_runner() -> None:
    ...
    try:
        ...
        print(f"Error: {e}")
        print("To install MCP support, run: pip install spectrafit[mcp]")
