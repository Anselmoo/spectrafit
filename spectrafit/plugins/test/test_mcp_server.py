...
class TestMCPServerInitialization:
    ...
    def test_mcp_not_available_error(self) -> None:
        ...
        with (
            patch("spectrafit.plugins.mcp_server.MCP_AVAILABLE", False),
            pytest.raises(ImportError, match="MCP dependencies are not available"),
        ):
            SpectraFitMCPServer()
...
class TestFitSpectrumTool:
    ...
    def sample_model(
        self,
    ) -> dict[str, dict[str, dict[str, dict[str, int | bool | float]]]]:
    ...
    def test_fit_spectrum_invalid_columns(
        self, mock_server: SpectraFitMCPServer
    ) -> None:
...
class TestGenerateFitModelTool:
    ...
    @pytest.mark.parametrize(
        "technique", ["raman", "uv-vis", "xps", "ir", "moessbauer"]
    )
    def test_technique_templates(
        self, mock_server: SpectraFitMCPServer, technique: str
    ) -> None:
...
class TestValidateSpectralDataTool:
    ...
    def test_technique_validation(
        ...
        if (
            "technique_specific" in result
            and "energy_range" in result["technique_specific"]
        ):
            assert (
                result["technique_specific"]["energy_range"]["passed"] == expected_valid
            )
...
class TestErrorHandling:
    ...
    def test_malformed_csv_data(self) -> None:
