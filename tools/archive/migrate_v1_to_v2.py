...
supported), converts the legacy ``peaks`` dict to a v2 ``[[components]]`` array,
and writes a clean v2-compatible TOML file.

This script is fully standalone — it does not import from ``spectrafit``.
...
# ---------------------------------------------------------------------------
# v1 format normalisation helpers (formerly spectrafit/models/migration.py)
# ---------------------------------------------------------------------------
...
def migrate_v1_format(data: dict[str, object]) -> dict[str, object]:
    """Normalise a v1.x input dict to a flat intermediate form.
    ...
    Returns:
        dict: Normalised dict ready for further v2 conversion.
    """
    if "fitting" in data:
